...
object WikipediaRanking {
   ...
   */
  //def indexAcc(langs: String, rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = ???
  //  rdd.flatMap( t =>  (langs, t))
...
//  def rankLRDBKAcc(lang: String, rdd: RDD[WikipediaArticle]): RDD[(String, WikipediaArticle)] =
//    rdd.filter(_.mentionsLanguage(lang)).groupBy()
...
//    lRDD.map( rddl => rddl.reduceByKey())

//    val listRDD = langs.map( lang => (lang, rdd.filter(t => t.mentionsLanguage(lang))) ) // List(RDD[WikipediaArticle])
  }
